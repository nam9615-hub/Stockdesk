...
export async function runPaper(market,entries,now=Date.now(),options={}){
  ...
  // A browser refresh can overlap the scheduled monitor. Re-read the winning
  // state once; its recent lastRun will normally turn this into a safe throttle.
  if(r.status===409 && options.conflictRetry!==true) return runPaper(market,entries,Date.now(),{...options,conflictRetry:true});
  if(!r.ok) throw new Error(r.status===409?'Paper conflict after retry: state preserved':`Paper write failed (${r.status})`);
