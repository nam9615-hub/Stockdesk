...
export function entrySignal(order, bars) {
  ...
  if(order.setup==='opening') {
    // Opening range is only an early-session tactic; later evaluate fresh setups.
    if(b.at-session[0].at>=60*60000) {
      const breakout=entrySignal({...order,setup:'breakout'},bars);
      return breakout.ok?{...breakout,reason:'장중 돌파·거래량 재확인'}:entrySignal({...order,setup:'pullback'},bars);
    }
